...
public class InAppPurchase
{
    ...
    public static final String IN_APP_CONSUMABLE_1_YEAR			= "consumable_1_year";
	...
	*/
	...
	private void sendConsumableDataToGoogle()
	{
		...
		{
			if(purchaseItem.getSku().equals(IN_APP_SUBSCRIPTION_1_MONTH) == false)
			{
	...
	public void purchaseItem(Activity activity, String skuCode)
	{
		...
		if(mIabHelper != null)
		{
			try
			{
				if(skuCode == IN_APP_SUBSCRIPTION_1_MONTH)
				{
					mIabHelper.launchPurchaseFlow(activity, skuCode,IabHelper.ITEM_TYPE_SUBS, REQUEST_CODE, mOnIabPurchaseFinishedListener, mPayLoad);
				}
				else
				{
	...
	private OnIabSetupFinishedListener mOnIabSetupFinishedListener = new OnIabSetupFinishedListener()
	{
		...
		public void onIabSetupFinished(IabResult result)
		{
			...
			if (result.isSuccess() == true)
			{
				...
				list.add(IN_APP_CONSUMABLE_1_YEAR);
	...
	/*private JSONObject refreshAuthCode(){
		...
		HttpThread ht = new HttpThread(REFRESH_AUTH_URL, HttpThread.POST_TYPE);
		ht.setParam(params);
		String jsonValue = ht.connction();
		JSONObject returnJsonObject = new JSONObject(jsonValue);
