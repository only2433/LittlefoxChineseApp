...
public class IntroLoadingActivity extends BaseActivity
{
	...
	private static final int MESSAGE_MARKET_ERROR					= 7;
    ...
    private void setupInAppPurchaseListener()
    {
    	mInAppPurchase.setOnBillingStatusListener(new IBillingStatusListener()
		{
			...
			public void onQueryInventoryFinished(IabResult result)
			{
				try
				{
					InAppInformation inappInformation = (InAppInformation) CommonUtils.getInstance(IntroLoadingActivity.this).getPreferenceObject(Common.PARAMS_INAPP_INFO, InAppInformation.class);
					String payType = InAppPurchase.IN_APP_FREE_USER;
					
					Log.f("PayInformation : "+ mInAppPurchase.getOwnedPurchaseItemList());
					...
					if(subscription != null)
					{
						...
						inappInformation = new InAppInformation(InAppPurchase.IN_APP_SUBSCRIPTION_1_MONTH, subscription.getPurchaseTime(), CommonUtils.getInstance(IntroLoadingActivity.this).getAdded1Month(subscription.getPurchaseTime()));
					...
					if(consumable != null)
					{
						...
						inappInformation = new InAppInformation(InAppPurchase.IN_APP_CONSUMABLE_1_MONTH, consumable.getPurchaseTime(), CommonUtils.getInstance(IntroLoadingActivity.this).getAdded1Month(consumable.getPurchaseTime()));
						CommonUtils.getInstance(IntroLoadingActivity.this).setPreferenceObject(Common.PARAMS_INAPP_INFO, inappInformation);
					...
					if(consumable != null)
					{
						...
						inappInformation = new InAppInformation(InAppPurchase.IN_APP_CONSUMABLE_1_YEAR, consumable.getPurchaseTime(), CommonUtils.getInstance(IntroLoadingActivity.this).getAdded1year(consumable.getPurchaseTime()));
						CommonUtils.getInstance(IntroLoadingActivity.this).setPreferenceObject(Common.PARAMS_INAPP_INFO, inappInformation);
	...
	private AsyncListener mAsyncListener = new AsyncListener()
	{
		...
		public void onRunningEnd(String code, Object mObject)
		{
			if(((BaseResult)mObject).getResult().equals(BaseResult.RESULT_OK))
			{
				...
				{
					updateProgress();
					UserTotalInformationObject object = new UserTotalInformationObject(((UserInformationResult)mObject).getUserInformation(), ((UserInformationResult)mObject).getChildInformationList());
